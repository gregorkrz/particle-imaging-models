#!/bin/bash
#SBATCH --job-name=scratch-pid-insseg-v1m2
#SBATCH --nodes=2
...
CONFIG="detector-v1m2-pt-v3m2-ft-pid-scratch"
...
COMMAND="sh ${TRAIN_PATH} -m 2 -g 4 -c panda/panseg/${CONFIG} -n ${CONFIG}-${MAX_LEN}-${EPOCH}-${CURRENT_DATETIME} -- --options data.train.max_len=${MAX_LEN} epoch=${EPOCH}"
