...
CONFIG="semseg-pt-v3m2-pilarnet-ft-5cls-fft"
...
COMMAND="sh ${TRAIN_PATH} -m 1 -g 2 -c panda/semseg/${CONFIG} -w ${CKPT_PATH} -n ${CONFIG}-${MAX_LEN}-${EPOCH}-${CURRENT_DATETIME} -- --options data.train.max_len=${MAX_LEN} epoch=${EPOCH}"
