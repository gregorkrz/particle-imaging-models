...
class AttentionRelationStep(Function):
    ...
    def forward(ctx, query, key, weight, index_target, index_refer):
        ...
        output = torch.zeros(m, g, dtype=torch.float32, device="cuda")
        attention_relation_step_forward_cuda(
    ...
    def backward(ctx, grad_output):
        ...
        n, g, c = query.shape
        m = index_target.shape[0]
        grad_query = torch.zeros(n, g, c, dtype=torch.float32, device="cuda")
        grad_key = torch.zeros(n, g, c, dtype=torch.float32, device="cuda")
        grad_weight = torch.zeros(c, dtype=torch.float32, device="cuda")
        attention_relation_step_backward_cuda(
            m,
            g,
            c,
...
class AttentionFusionStep(Function):
    ...
    def forward(ctx, weight, value, index_target, index_refer):
        ...
        output = torch.zeros(n, g, c, dtype=torch.float32, device="cuda")
        attention_fusion_step_forward_cuda(
    ...
    def backward(ctx, grad_output):
        ...
        n, g, c = value.shape
        m = index_target.shape[0]
        grad_weight = torch.zeros(m, g, dtype=torch.float32, device="cuda")
        grad_value = torch.zeros(n, g, c, dtype=torch.float32, device="cuda")
        attention_fusion_step_backward_cuda(
            m,
            g,
            c,
