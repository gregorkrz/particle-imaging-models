...
enc_total = sum(enc_depths)
dec_total = sum(dec_depths)

param_dicts = []

# encoder: exponent dec_total-1..enc_total+dec_total-2+; smallest LR at first encoder block.
for e in range(len(enc_depths)):
    for b in range(enc_depths[e]):
        exp = (enc_total - sum(enc_depths[:e]) - b - 1) + dec_total
        param_dicts.append(
            dict(
...
del enc_depths, dec_depths, enc_total, dec_total, backbone_mult
