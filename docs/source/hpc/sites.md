...
executor follows the verb, *not* the site name - so `pimm launch --site mycluster`
runs on the current node **inside the mycluster container**, and `pimm submit --site
mycluster` queues the same environment on Slurm.

## Add your own site (start from `slurm`)

For a generic Slurm cluster, build from the **`slurm`** base.
Drop a `launch/sites/<name>.yaml` that inherits it (`--site <name>` resolves to `launch/sites/<name>.yaml`):
...
The keys a site profile accepts:
