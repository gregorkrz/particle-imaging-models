...
into the ragged `(N, C)` + `offset` batch the models consume.
...
If you read one page first, read {doc}`data_format` - the `(N, C)` + `offset` format is the single idea that lets the same models run on argon TPCs, water
Cherenkov detectors, and wire-plane data.

- {doc}`Data format <data_format>` - the `(N, C)` + cumulative `offset` format, and how `collate_fn` builds it.
