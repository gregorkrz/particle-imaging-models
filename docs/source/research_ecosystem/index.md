...
- {doc}`Sharing & using trained models <using_trained_models>` — load any export
  with `pimm.from_pretrained`, reproduce the exact input a model expects, and
...
- {doc}`Contributing a dataset <contributing_a_dataset>` — write a reader and a
  dataset class that emit the packed-batch contract.
...
the **one-forward trainer contract**. If you haven't yet, read
