...
Logging hooks write to the shared trainer channels `trainer.storage` and `trainer.writer` described in {doc}`the shared channels table <index>`, plus one string channel specific to console output:
...
* - `trainer.comm_info["iter_info"]`
  - A string assembled per step for the console; the logger that prints the one-line status reads it.
```

Write a scalar to `trainer.storage` when you want {py:class}`~pimm.engines.hooks.logging.InformationWriter` to average it over an epoch and print it; write directly to `trainer.writer` when you want an explicit named curve in TensorBoard/W&B.
...
The primary logging hook: it collects per-step scalars from the model output and renders both the per-step console line and the end-of-epoch averages.
...
The resulting console line combines the `IterationTimer` fields with the filtered scalars:
