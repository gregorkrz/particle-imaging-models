"""Panda detector v3m2 with shared queries for multiple label sets."""
...
class MaskQueryDecoder(nn.Module):
    ...
    def _get_queries(
        ...
        if self.query_type == "learned":
            ...
            counts = torch.full(
                (batch_size,), max_queries, dtype=torch.int32, device=device
            )
    ...
    def _expand_masks(
        self, mask_logits: torch.Tensor, thing_mask: torch.Tensor
    ) -> torch.Tensor:
    ...
    def _forward_decoder(self, point: Point, return_aux: bool = False):
        ...
        for blk in self.blocks:
            ...
            if mask_logits is not None:
                ...
                final_mask_embed = (
                    mask_embed * query_valid_f if mask_embed is not None else None
                )
...
def _replace_instance_loss(cfg):
    """Select the optimized loss used by detector-v3m2."""
    cfg = deepcopy(cfg)
    ...
    def forward(self, point: Point, return_decoder: bool = False):
        ...
        for label in self.labels:
            ...
            if label in self.stuff_head_by_label:
                stuff_logits = self.stuff_head_by_label[label](point_full.feat).squeeze(
                    -1
                )
...
class MultiLabelDetectorV3M2(PointModel):
    ...
    def __init__(
        ...
        self.criteria_by_label = {
            label: build_criteria(
                self._criteria_cfg(label, criteria, criteria_by_label)
            )
            for label in self.labels
        }
        ...
        if train_filter_use_gt:
            raise ValueError(
                "detector-v3m2 batches queries across labels and does not support "
    ...
    def forward(self, input_dict, return_point=False, return_decoder=False):
        ...
        decoder_result = self.decoder(
            Point(point_full.copy()), return_decoder=return_decoder
        )
        if return_decoder:
        ...
        if (
            return_decoder
            and decoder_outputs is not None
            and "decoder" not in return_dict
        ):
            return_dict["decoder"] = decoder_outputs
