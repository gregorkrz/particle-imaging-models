"""
General checkpoint loading utilities for pimm models.
...
def remap_state_dict_keys(
    ...
    *,
    ...
    Args:
        state_dict: Source state dictionary.
        key_mapping: Dictionary mapping old keys or prefixes to new keys or
            prefixes. Exact key matches take priority. Prefix mappings are
            applied when a state-dict key starts with the source string.
    ...
    for k, v in state_dict.items():
        if k in key_mapping:
            ...
            for source, target in key_mapping.items():
                if source and k.startswith(source):
                    new_key = f"{target}{k[len(source):]}"
                    break
            if new_key is None:
