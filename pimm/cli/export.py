...
def _default_training_config(run_dir: Path | None, config_path: Path) -> Any:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--private", action="store_true", help="Create a private Hub repo when pushing.")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Write pytorch_model.bin instead of model.safetensors.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if config_path is None:
        raise ValueError("--config is required when --run-dir does not contain config.py")
    ...
    if args.push_to_hub:
        push_to_hub(output_dir, args.push_to_hub, private=args.private, token=args.token)
